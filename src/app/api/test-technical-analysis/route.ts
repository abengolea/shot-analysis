...
export async function POST(request: NextRequest) {
  try {
    ...
    ASCENSO:
    7. Mano no dominante guía el balón (¿mano guía visible?) - PESO: 2.07%
    8. Codos cerca del cuerpo (¿codos no abiertos?) - PESO: 6.88%
    9. Ángulo de codo estable en ascenso (¿mantiene ángulo hasta set point?) - PESO: 5%
    10. Balón sube en línea recta (¿trayectoria recta?)
    11. Trayectoria suave al set point (¿movimiento fluido?)
    12. Set point sobre la cabeza (¿balón sobre cabeza?) - PESO: 7.86%
    13. Timing correcto (¿no muy rápido/lento?)

    FLUIDEZ (PESO: 47.5% - CRÍTICO):
    14. Tiro en un solo movimiento continuo (¿movimiento fluido?)
    15. Sincronización piernas-brazos (¿todo junto?)

    LIBERACIÓN:
    16. Mano guía se retira a tiempo (¿mano guía se quita?) - PESO: 3.10%
    17. Extensión completa del brazo (¿brazo extendido?)
    18. Muñeca con snap hacia abajo (¿muñeca activa?)
    19. Ángulo de salida apropiado (¿ángulo bueno?)

    SEGUIMIENTO:
    20. Mantiene follow-through (¿follow-through visible?)
    21. Equilibrio al aterrizar (¿aterriza equilibrado?)
    22. Duración del follow-through (¿mantiene 1-2 seg?)
    23. Consistencia general del movimiento (¿movimiento consistente?)

    📋 REGLAS PARA RECOMENDACIONES AUTOMÁTICAS (por peso):
    - Si FLUIDEZ < 7: PRIORIDAD MÁXIMA (47.5% peso)
    - Si SET POINT < 6: PRIORIDAD ALTA (7.86% peso)
    - Si CODO < 6: PRIORIDAD ALTA (6.88% peso)
    - Si ÁNGULO CODO ESTABLE < 6: PRIORIDAD ALTA (5% peso)
    - Si MANO LIBERACIÓN < 5: PRIORIDAD MEDIA (3.10% peso)
    - Si MANO ASCENSO < 5: PRIORIDAD MEDIA (2.07% peso)
