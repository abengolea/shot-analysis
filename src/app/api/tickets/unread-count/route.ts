...
export async function GET(request: NextRequest) {
  try {
    ...
    if (who.isAdmin) {
      // Para admin: contar tickets con unreadForAdmin > 0
      const qs = await adminDb.collection('tickets').where('unreadForAdmin', '>', 0).limit(1_000).get();
      return NextResponse.json({ unread: qs.size });
    } else {
      // Para usuario: contar tickets del usuario con unreadForUser > 0
    ...
    console.error('tickets unread-count GET error', e);
