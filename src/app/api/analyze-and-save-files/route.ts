...
import { analyzeBasketballShotWithFilesAPI } from '@/lib/gemini-files-api';
import { adminStorage, adminDb } from '@/lib/firebase-admin';
import { z } from 'zod';

const AnalyzeAndSaveFilesSchema = z.object({
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await Promise.race([
      performAnalysisWithFilesAPI(request),
      timeoutPromise
    ]);
...
async function performAnalysisWithFilesAPI(request: NextRequest) {
  ...
  // Validar inputs
  const validation = AnalyzeAndSaveFilesSchema.safeParse({ ageCategory, playerLevel, shotType, playerId });
  if (!validation.success) {
    return NextResponse.json({ error: 'Datos de formulario inválidos', details: validation.error.errors }, { status: 400 });
  }

      // 1. ANÁLISIS CON GEMINI FILES API
  const videoBuffer = Buffer.from(await videoFile.arrayBuffer());
  const analysisResult = await analyzeBasketballShotWithFilesAPI(
    videoBuffer,
    videoFile.name,
    ageCategory,
    playerLevel,
  ...
  await gcsFile.save(videoBuffer, {
    metadata: {
      ...
      metadata: {
        ...
        playerLevel,
        shotType,
        analysisMethod: 'gemini-files-api',
      },
    },
  });
  ...
  // Filtrar videoBase64 del análisis
  const { videoBase64, ...cleanTechnicalAnalysis } = analysisResult.technicalAnalysis || {};
  ...
  const analysisData = {
    ...
    analysisMethod: 'gemini-files-api',
    geminiFileUri: analysisResult.fileUri, // URI del archivo en Gemini
  };
  ...
  await pendingRef.set(pendingData);
    ...
    analysisId, 
    publicUrl, 
    analysis: cleanTechnicalAnalysis,
    message: 'Análisis completado con Files API y guardado',
